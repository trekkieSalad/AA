...
save("dict.jld2", Dict("train_imgs" => inputs[trainInd], "train_labels" => targets[trainInd], "test_imgs" => inputs[testInd], "test_labels" => targets[testInd]))

#train_imgs = inputs[trainInd];
#test_imgs = inputs[testInd];
#train_labels = targets[trainInd];
#test_labels = targets[testInd];

train_imgs   = load("dict.jld2", "train_imgs");
train_labels = load("dict.jld2", "train_labels");
test_imgs    = load("dict.jld2", "test_imgs");
test_labels  = load("dict.jld2", "test_labels");
...
function convertirArrayImagenesHWCN(imagenes)
    ...
    nuevoArray = Array{Float32,4}(undef, 100, 100, 1, numPatrones); # Importante que sea un array de Float32
    for i in 1:numPatrones
        @assert (size(imagenes[i])==(100,100)) "Las imagenes no tienen tamaño 50x50";
...
ann = Chain(Conv((3, 3), 1=>16, pad=(1,1), funcionTransferenciaCapasConvolucionales),
            MaxPool((2,2)),
...
mejorPrecision = mean(accuracy.(train_set));
criterioFin = false;
numCiclo = 0;
...
while (!criterioFin)
    ...
    if (precisionEntrenamiento > mejorPrecision) #quitamos >= por si da siempre lo mismo
